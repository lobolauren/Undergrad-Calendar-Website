...
def save_graph_to_pdf(graph, filename):
    graph.format = 'pdf'
...
def add_outside_department_course(graph, outside_course):
...
def makegraph(course_data):
    ...
    try:
        course_attr = name
        if '*' in name: # only get this if the input is a full course name
    ...
    if len(name) > 4:
        parsing_prereq(course_graph, name, course_data)
    else:
        parsing_department(course_graph, name,course_data)

    # file naming
    # remove special characters for filename
    filename = name+"-graph"
    filename_query = input("Name graph file? [y/n]: ")
    if filename_query.lower() == "y" or filename_query.lower() == "yes":
        filename = input("Enter graph name: ")
    ...
    save_graph_to_pdf(course_graph, filename)

    # Course code (ex. CIS*1300 or CIS or 1300)
    while True:
        continue_search = input("\n Graph another course? [y/n] ")
        if continue_search.lower() == "n" or continue_search.lower() == "no":
            break
